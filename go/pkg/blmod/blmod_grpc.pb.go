...
func (c *labsModulesServiceClient) RunModule(ctx context.Context, in *RunModuleRequest, opts ...grpc.CallOption) (*RunModuleResponse, error) {
	out := new(RunModuleResponse)
	err := c.cc.Invoke(ctx, "/blmod.v1.LabsModulesService/RunModule", in, out, opts...)
	if err != nil {
		return nil, err
	}
	return out, nil
...
func (UnimplementedLabsModulesServiceServer) RunModule(context.Context, *RunModuleRequest) (*RunModuleResponse, error) {
...
func _LabsModulesService_RunModule_Handler(srv interface{}, ctx context.Context, dec func(interface{}) error, interceptor grpc.UnaryServerInterceptor) (interface{}, error) {
	in := new(RunModuleRequest)
	if err := dec(in); err != nil {
		return nil, err
	}
	if interceptor == nil {
		return srv.(LabsModulesServiceServer).RunModule(ctx, in)
	}
	info := &grpc.UnaryServerInfo{
		Server:     srv,
		FullMethod: "/blmod.v1.LabsModulesService/RunModule",
	}
	handler := func(ctx context.Context, req interface{}) (interface{}, error) {
		return srv.(LabsModulesServiceServer).RunModule(ctx, req.(*RunModuleRequest))
	}
	return interceptor(ctx, in, info, handler)
}
